import torch
import numpy as np
from utils.math import qmul, qrot, matrix_to_cont6D, cont6d_to_matrix, qinv
...
class Skeleton:
    ...
    def fk_local_quat(self, local_quats, root_pos):
        ...
        for i in range(1, len(self.parents)):
            global_quats[:, i] = qmul(
                global_quats[:, self.parents[i]], local_quats[:, i]
            )
            global_pos[:, i] = (
                qrot(global_quats[:, self.parents[i]], offsets[:, i])
                + global_pos[:, self.parents[i]]
            )
        return global_quats, global_pos

    def fk_local_quat_np(self, local_quats, root_pos):
        global_quats, global_pos = self.fk_local_quat(
            torch.from_numpy(local_quats).float(), torch.from_numpy(root_pos).float()
        )
    ...
    def fk_global_quat_np(self, global_quats, root_pos):
        global_pos = self.fk_global_quat(
            torch.from_numpy(global_quats).float(), torch.from_numpy(root_pos).float()
        )
    ...
    def fk_local_cont6d(self, local_cont6d, root_pos):
        ...
        for i in range(1, len(self.parents)):
            global_cont6d_mat[:, i] = torch.matmul(
                global_cont6d_mat[:, self.parents[i]].clone(), local_cont6d_mat[:, i]
            )
            global_pos[:, i] = (
                torch.matmul(
                    global_cont6d_mat[:, self.parents[i]], offsets[:, i].unsqueeze(-1)
                ).squeeze()
                + global_pos[:, self.parents[i]]
            )
    ...
    def fk_local_cont6d_np(self, local_cont6d, root_pos):
        global_cont6d, global_pos = self.fk_local_cont6d(
            torch.from_numpy(local_cont6d).float(), torch.from_numpy(root_pos).float()
        )
    ...
    def fk_global_cont6d_np(self, global_cont6d, root_pos):
        global_pos = self.fk_global_cont6d(
            torch.from_numpy(global_cont6d).float(), torch.from_numpy(root_pos).float()
        )
        return global_pos.cpu().numpy()
    ...
    def global_to_local_quat(self, global_quat):
        ...
        for i in range(1, len(self.parents)):
            local_quat[:, i] = qmul(
                qinv(global_quat[:, self.parents[i]]), global_quat[:, i]
            )
        return local_quat
